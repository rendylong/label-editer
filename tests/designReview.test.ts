...
function fakeCapture(overrides: Record<string, unknown> = {}) {
  return vi.fn(async ({ width, height, areas, pxPerMm }: any) => ({
    front: { bytes: PNG, width, height }, back: { bytes: PNG, width, height },
    areas: Object.fromEntries(areas.filter((area: any) => area.carrier !== 'bare').map((area: any) => [area.id, {
      bytes: PNG, width: Math.round(area.artboard.widthMm * pxPerMm), height: Math.round(area.artboard.heightMm * pxPerMm),
    }])),
    ...overrides,
  }))
}
...
describe('blueprint-derived design review', () => {
  ...
  it('fails closed when a declared font asset cannot be loaded by Chromium', async () => {
    const root = await temporaryDirectory()
    const source = blueprint()
    const brokenFont = Buffer.from('wOF2')
    source.assets.push({
  ...
  it('renders an applied transparent substrate as declared film rather than inventing an opaque panel', () => {
    const source = blueprint()
    source.areas[1].substrate = { kind: 'transparent', opacity: 0.2, boundary: { shape: 'rectangle' }, material: 'clear film' }

    const html = renderBlueprintHtml(source, { pxPerMm: 5, width: 640, height: 480, assets: new Map() })
    const back = html.slice(html.indexOf('data-side="back"'))

    expect(back).toContain('carrier-film-extent')
    expect(back).not.toContain('carrier-panel--opaque')
  })
  ...
  it('publishes safe manifest paths that exactly match sanitized area files', async () => {
    ...
    await expect(readFile(path.join(outputDir, paths[0]))).resolves.toEqual(PNG)
    await expect(readFile(path.join(outputDir, paths[1]))).resolves.toEqual(PNG)
  })
