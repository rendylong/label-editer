...
describe('工艺领域规则', () => {
  ...
  it('文字可见图形和遮罩共享同一个中心锚点与竖排旋转', () => {
    ...
    expect(layout).toEqual({ width: 120, height: 120, rotation: 105, lines: ['AB'] })
  ...
  it('按文本框宽度对中英文自动换行，并保留用户输入的显式换行', () => {
    ...
    expect(english).toEqual({ width: 30, height: 240, rotation: 0, lines: ['A B', 'C'] })
    expect(chinese).toEqual({ width: 30, height: 240, rotation: 0, lines: ['香水标', '签设计'] })
    expect(explicit).toEqual({ width: 30, height: 240, rotation: 0, lines: ['AB', 'CD'] })
  })
